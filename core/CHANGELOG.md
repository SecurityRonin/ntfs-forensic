# Changelog

All notable changes to `ntfs-core` (the reader) are documented here. The format
follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and the project
adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [0.9.8](https://github.com/SecurityRonin/ntfs-forensic/compare/ntfs-core-v0.9.7...ntfs-core-v0.9.8) - 2026-08-24

### Added

- *(vfs)* classify NTFS special files in both Linux encodings ([#15](https://github.com/SecurityRonin/ntfs-forensic/pull/15))

## [0.9.7](https://github.com/SecurityRonin/ntfs-forensic/compare/ntfs-core-v0.9.6...ntfs-core-v0.9.7) - 2026-08-24

### Added

- *(vfs)* surface NTFS reparse-point and ntfs-3g symlinks

### Documentation

- *(vfs)* clarify IntxLNK version-byte handling; tidy

### Fixed

- *(vfs)* bound a decoded symlink target by the caller's cap
- *(vfs)* read a symlink's PathBuffer past Flags, at data + 12

## [0.9.6](https://github.com/SecurityRonin/ntfs-forensic/compare/ntfs-core-v0.9.5...ntfs-core-v0.9.6) - 2026-07-22

### Added

- *(ntfs-vfs)* GREEN — enumerate unallocated clusters from $Bitmap

## [0.9.5](https://github.com/SecurityRonin/ntfs-forensic/compare/ntfs-core-v0.9.4...ntfs-core-v0.9.5) - 2026-07-20

### Added

- *(vfs)* implement `FileSystem::volume_label()` — decode the `$Volume` metafile's `$VOLUME_NAME` attribute (UTF-16LE)

### Fixed

- *(deps)* bump forensic-vfs 0.5 -> 0.7

## [0.9.4](https://github.com/SecurityRonin/ntfs-forensic/compare/ntfs-core-v0.9.3...ntfs-core-v0.9.4) - 2026-07-19

### Fixed

- *(deps)* bump forensic-vfs 0.4.3 -> 0.5

## [0.9.3](https://github.com/SecurityRonin/ntfs-forensic/compare/ntfs-core-v0.9.2...ntfs-core-v0.9.3) - 2026-07-19

### Added

- *(vfs)* GREEN — implement deleted_nodes() real MFT recovery

## [0.9.2]

- Current published reader: pure-Rust, `forbid(unsafe)`, panic-free-by-lint,
  input-fuzzed NTFS parser — boot sector, MFT records and attributes, runlists,
  index buffers, attribute lists, and LZNT1 decompression.

<!-- release-plz appends new versions above this line, newest first. -->
