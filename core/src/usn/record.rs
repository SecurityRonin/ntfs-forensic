...
pub fn parse_usn_journal(data: &[u8]) -> Result<Vec<UsnRecord>> {
    ...
    while offset + 8 <= len {
        ...
        if !(USN_V4_MIN_SIZE..=USN_MAX_RECORD_SIZE).contains(&record_len) {
            // Invalid record length — scan forward a word and retry.
            offset += 8;
            continue;
        }

        if offset + record_len > len {
            // Record header claims more bytes than remain — stop.
            break;
        }
        ...
        match major_version {
            ...
            _ => {
                // Unknown version — scan forward a word and retry.
                offset += 8;
                continue;
            }
        }
...
mod tests {
    ...
    fn test_parse_journal_skips_invalid_unknown_and_v4_records() {
        // Exercises the scan-forward paths: invalid record length, unknown
        // version, V4 skip, and a record whose length runs past end-of-data.
        let mut data = Vec::new();

        // 1. Invalid record length (too small)
        ...
        // 4. Record that extends past end of data
