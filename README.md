...
- **Validated on real artifacts against independent oracles** — never against fixtures we hand-encoded and graded ourselves. The boot parser is cross-checked against **The Sleuth Kit** (`fsstat`) on the DEF CON 2018 `MaxPowers` image; LZNT1 decode against **TSK** (`icat`) on a real compressed stream from the DFIR Madness "Stolen Szechuan Sauce" CITADEL-DC01 image; `$MFT` parsing against the independent **`mft` crate**; the `$LogFile` RCRD reader against **TSK** input extraction plus a differential page census (4470/4470 pages on DC01); and the `$LogFile` redo/undo operation-code vocabulary against **LogFileParser** (jschicht, via Wine) — the same tool whose 47k-row per-record output is the differential oracle for the in-progress transaction decoder. Full evidence, tiers, corpora, and reproduction steps: **[docs/validation.md](docs/validation.md)**.
